...
def run():
	...
	dataloader = DataLoader(dataset,
	                        ...
	                        drop_last=True)
	print('data loaded.') # added
	latent_mask = [int(l) for l in opts.latent_mask.split(",")]
	if opts.n_images is None:
		print('n_image is None.')
		opts.n_images = len(dataset)

	global_i = 0
	print('begining to generate images:') # added

	for input_batch in tqdm(iter(dataloader)): # edited: tqdm(dataloader):
		...
		with torch.no_grad():
			...
			for image_idx, input_image in enumerate(input_batch):
				...
				print('\n\n opts.separate_n_images is', opts.separate_n_images, '\n\n')


				if not opts.separate_n_images: 
					...
					print('saving results to: ', os.path.join(mixed_path_results, os.path.basename(input_im_path)))
					Image.fromarray(res).save(os.path.join(mixed_path_results, os.path.basename(input_im_path)))

				else:
					print('separating output:')
					
					for i, output in enumerate(multi_modal_outputs):
						res1 = res.copy()
						output = tensor2im(output)
						res1 = np.concatenate([res1, np.array(output.resize(resize_amount))], axis=1)
						print('saving results to: ',os.path.join(mixed_path_results, os.path.basename(input_im_path)))
						Image.fromarray(res1).save(os.path.join(mixed_path_results, os.path.basename(input_im_path))[:-4]+f'_style_{i}.png')
